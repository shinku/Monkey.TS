...
var canvasLayerOption = (function () {
    function canvasLayerOption(canvas, img, x, y, isMask) {
        if (x === void 0) { x = 0; }
        if (y === void 0) { y = 0; }
        ...
        if (!img.onload) {
            img.onload = function (e) {
                _this.onImgLoaded();
                _this._width = e.target.width;
                _this._height = e.target.height;
            };
        }
        else {
            this.onImgLoaded();
    ...
    Object.defineProperty(canvasLayerOption.prototype, "center", {
        get: function () {
            function p(x, y) {
                this.x = x;
                this.y = y;
    ...
    canvasLayerOption.prototype.canvasAddLayer = function () {
        if (this._canvas) {
            this._canvas.addLayer(this);
        }
    };
    Object.defineProperty(canvasLayerOption.prototype, "ismask", {
        get: function () {
            return this._ismask;
        },
        enumerable: true,
    ...
    Object.defineProperty(canvasLayerOption.prototype, "bound", {
        get: function () {
            return {
                x: this.posx,
                y: this.posy,
    ...
    canvasLayerOption.prototype.hitTest = function (x, y) {
        var ret = this.bound;
...
var MF_Canvas = (function (_super) {
    ...
    function MF_Canvas() {
        ...
        _this.offsety = 0;
        _this._rotate = 0;
        _this.offsetrotate = 0;
        _this.imgPosition = {};
        _this.layers = [];
        _this.fillStyle = 'normal';
        _this.canvas = document.createElement('canvas');
        _this.context = _this.canvas.getContext('2d');
        return _this;
    }
    MF_Canvas.prototype.getStyle = function (styename) {
        return this.myCanvas.style[styename];
    };
    Object.defineProperty(MF_Canvas.prototype, "mask", {
        set: function (m) {
            this._masker = m;
        },
        enumerable: true,
    ...
    Object.defineProperty(MF_Canvas.prototype, "background", {
        set: function (layer) {
    ...
    MF_Canvas.prototype.drawMask = function () {
    };
    ...
    MF_Canvas.prototype.chekTarget = function (x, y) {
        var i = 0;
        var hittarget = null;
    ...
    MF_Canvas.prototype.removeLayer = function (layer) {
        for (var i = 0; i < this.layers.length; i++) {
    ...
    MF_Canvas.prototype.setLayerToTheTop = function (layer) {
        for (var i = 0; i < this.layers.length; i++) {
            if (layer == this.layers[i]) {
                this.layers.splice(i, 1);
                continue;
...
var backgroundCanvasLayOption = (function (_super) {
    __extends(backgroundCanvasLayOption, _super);
    function backgroundCanvasLayOption(canvas, img, x, y) {
        if (x === void 0) { x = 0; }
        if (y === void 0) { y = 0; }
        var _this = _super.call(this, canvas, img, x, y, false) || this;
        canvas.background = _this;
        _this._isbackground = true;
        return _this;
    }
    backgroundCanvasLayOption.prototype.canvasAddLayer = function () {
    };
    return backgroundCanvasLayOption;
}(canvasLayerOption));
var MF_ImageScroller = (function (_super) {
    ...
    function MF_ImageScroller(_boxnametag, width, height) {
        ...
        if (typeof _boxnametag == 'string') {
            _this.box = document.getElementById(_boxnametag);
        }
        else if (_boxnametag['tagName']) {
            _this.box = _boxnametag;
        }
        _this.mf_canvas = new MF_Canvas();
        _this.mf_canvas.setStyle('width', width + "px");
        _this.mf_canvas.setStyle('height', height + "px");
        _this.box.appendChild(_this.mf_canvas.myCanvas);
        MF_VFILE.getInstance().addEventListener('fileloaded', function (e) {
            _this.handleFileLoaded(e);
        });
        _this.mf_canvas.addEventListener('drawcomplete', function (e) {
            _this.dispatchEvent(new MF_EVENT.Event('drawcomplete'));
        });
        return _this;
    }
    Object.defineProperty(MF_ImageScroller.prototype, "CanvasFilleStyle", {
        set: function (val) {
    ...
    MF_ImageScroller.prototype.addMask = function (img, x, y) {
        var layer = new canvasLayerOption(this.mf_canvas, img, x, y, true);
    };
    MF_ImageScroller.prototype.addBackGround = function (img, x, y) {
        x = x || 0;
        y = y || 0;
        var layer = new backgroundCanvasLayOption(this.mf_canvas, img, x, y);
        return layer;
    };
    MF_ImageScroller.prototype.hitTest = function (x, y) {
        return this.mf_canvas.chekTarget(x, y);
    };
    MF_ImageScroller.prototype.setLayerToTheTop = function (layer) {
    ...
    MF_ImageScroller.prototype.update = function () {
        this.mf_canvas.update();
    };
    MF_ImageScroller.prototype.removeLayer = function (layer) {
...
var MF_ImageScrollControler = (function (_super) {
    ...
    function MF_ImageScrollControler(parenttagname, width, height, useFc, rate) {
        if (useFc === void 0) { useFc = true; }
        if (rate === void 0) { rate = 1; }
        var _this = _super.call(this, parenttagname, width * rate, height * rate) || this;
        _this.mf_canvas.setStyle('transform-origin', '0 0');
        _this.mf_canvas.setStyle('-webkit-transform-origin', '0 0');
        _this.mf_canvas.setStyle('-o-transform-origin', '0 0');
        _this.mf_canvas.setStyle('-moz-transform-origin', '0 0');
        var r = 1 / rate;
        _this.mf_canvas.setStyle('transform', 'scale(' + r + ',' + r + ')');
        _this.mf_canvas.setStyle('-webkit-transform', 'scale(' + r + ',' + r + ')');
        _this.mf_canvas.setStyle('-o-transform', 'scale(' + r + ',' + r + ')');
        _this.mf_canvas.setStyle('-moz-transform', 'scale(' + r + ',' + r + ')');
        if (!useFc)
            return _this;
        _this.fc = new MF_FACETOUCH(_this.box);
        _this.fc.addEventListener('tapmove', function (e) {
            _this.handleMove(e.data);
        });
        _this.fc.addEventListener('taprotate', function (e) {
...
var MF_Loading = (function (_super) {
    ...
    function MF_Loading(p) {
        ...
        _this.parameter = p;
        _this.srcNames = _this.parameter.srcNames ? _this.parameter.srcNames : "alts";
        _this.loadUp = typeof (_this.parameter.LDup) === "function" ? _this.parameter.LDup : null;
        _this.callback = typeof (_this.parameter.LDsuccess) === "function" ? _this.parameter.LDsuccess : null;
        if (_this.parameter.LDpage) {
            _this.getDomList(document);
        }
        ;
        if (_this.parameter.LDdom) {
            _this.addLoadObject(p.LDdom);
        }
        ;
        _this.upDate(0);
        return _this;
...
var MF_animateImages = (function (_super) {
    ...
    function MF_animateImages(p) {
        ...
        _this.up = null;
        _this.rmsPrefix = /^-ms-/;
        _this.rdashAlpha = /-([\da-z])/gi;
        _this.cssPrefixes = ["Webkit", "O", "Moz", "ms"];
        if (_this.parameter.imgList.length > 0) {
            _this.width = _this.parameter.width ? _this.parameter.width : _this.parameter.imgList[0].width;
            _this.height = _this.parameter.height ? _this.parameter.height : _this.parameter.imgList[0].height;
        }
        if (MF_display_Judger.isDom(_this.parameter.parents))
            _this.parents = _this.parameter.parents;
        else if (MF_display_Judger.isJQ(_this.parameter.parents))
            _this.parents = _this.parameter.parents[0];
        _this.anmtList = _this.parameter.imgList;
        _this.type = _this.parameter.type ? _this.parameter.type : "canvas";
        _this.step = _this.parameter.step || 1;
        _this.times = _this.parameter.times || 100;
        _this.loop = _this.parameter.loop || false;
        return _this;
    }
...
var MF_animateSheet = (function (_super) {
    ...
    function MF_animateSheet(p) {
        var _this = this;
        _this.parameter = p;
        if (MF_display_Judger.isLdCompleteList(_this.parameter.imgList)) {
            _this.parameter.imgList = MF_display_Judger.getImgList(_this.parameter.imgList);
        }
        else {
            _this.parameter.imgList = [];
        }
        _this = _super.call(this, _this.parameter) || this;
        _this.length = _this.parameter.imgList.length;
        if (_this.type == "canvas") {
            _this.canvas = _this.creatElment("canvas");
            _this.canvas.width = _this.width;
            _this.canvas.height = _this.height;
            _this.parents.appendChild(_this.canvas);
        }
        else if (_this.type == "img") {
        }
        _this.endIndex = _this.length;
        _this.toProgress(_this.progress);
        return _this;
    }
    ;
...
var MF_animateSprite = (function (_super) {
    ...
    function MF_animateSprite(p) {
        var _this = this;
        _this.parameter = p;
        if (MF_display_Judger.isDom(_this.parameter.imgList)) {
            _this.parameter.imgList = [_this.parameter.imgList];
        }
        else if (MF_display_Judger.isLdCompleteList(_this.parameter.imgList)) {
            _this.parameter.imgList = MF_display_Judger.getImgList(_this.parameter.imgList);
        }
        else if (!MF_display_Judger.isJQ(_this.parameter.imgList)) {
            _this.parameter.imgList = [];
        }
        _this = _super.call(this, p) || this;
        _this.length = _this.parameter.row * _this.parameter.column;
        var _img = _this.anmtList[0];
        _this.getPosition = function (n) {
            return {
        ...
        if (_this.type == "canvas") {
            ...
            var box = _this.creatElment("div");
            _this.css(box, { position: "absolute", top: 0, left: 0, width: _this.width + "px", height: _this.height + "px", overflow: "hidden" }, null);
            _this.parents.appendChild(box);
            box.appendChild(_img);
            _this.css(_img, { position: "absolute", top: 0, left: 0 }, null);
            _this.drawImages = function (n) {
                var _position = this.getPosition(n);
...
var MF_PackInterface = (function (_super) {
    ...
    function MF_PackInterface(EVENT_OBJ) {
        var _this = _super.call(this) || this;
        _this.ajaxer = new MF_DoAjax();
        _this.EVENT_OBJ = EVENT_OBJ || MF_NET.GET_IN();
        _this.init();
        return _this;
    }
...
var MF_FACETOUCH = (function (_super) {
    ...
    function MF_FACETOUCH(targetname) {
        ...
        _this.element = null;
        _this.isTouch = false;
        _this.touchpoints = new Array();
        _this.name = 'shin';
        _this.initdis = 0;
        _this.initrotation = 0;
        _this.pointsnumber = 0;
        _this.lastdis = 0;
        _this.lastrotation = 0;
        _this.element = document.getElementById(targetname) || targetname || document.body;
        _this.initEvent();
        return _this;
    }
    ;
